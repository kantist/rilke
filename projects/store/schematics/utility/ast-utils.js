...
exports.addStateDeclarationToModule =
	exports.getStoreModuleDeclaration =
	exports.addRouteDeclarationToModule =
	exports.addRedirectRouteDeclarationToModule =
	exports.getRouterModuleDeclaration =
	exports.addFeatureRouteForLayout =
	exports.getEnvironmentExportName =
	exports.isImported =
	exports.addBootstrapToModule =
	exports.addExportToModule =
	exports.addProviderToModule =
	exports.addImportToModule =
	exports.addDeclarationToModule =
	exports.addSymbolToNgModuleMetadata =
	exports.getMetadataField =
	exports.getDecoratorMetadata =
	exports.insertAfterLastOccurrence =
	exports.findNode =
...
const ts = require('../third_party/files/typescript');
...
function addSymbolToNgModuleMetadata(source, ngModulePath, metadataField, symbolName, importPath = null) {
	...
	if (matchingProperties.length == 0) {
		...
		if (expr.properties.length == 0) {
			...
			toInsert = `\n  ${metadataField}: [\n${(0, helper_1.indentBy)(4)`${symbolName}`}\n  ]\n`;
		} else {
			...
			if (matches) {
				toInsert =
					...
					`${(0, helper_1.indentBy)(matches[2].length + 2)`${symbolName}`}${matches[0]}]`;
			} else {
	...
	if (node.kind == ts.SyntaxKind.ArrayLiteralExpression) {
		...
		position--;
		toInsert = `\n${(0, helper_1.indentBy)(2)`${symbolName}`}\n	`;
	} else {
		// Get the indentation of the last element, if any.
		const text = node.getFullText(source);
		const matches = text.match(/^(\r?\n)(\s*)/);
		if (matches) {
			toInsert = `,${matches[1]}${(0, helper_1.indentBy)(matches[2].length)`${symbolName}`}`;
		} else {
			toInsert = `, ${symbolName}`;
		}
	}
 ...
 */
function addFeatureRouteForLayout(source, fileToAdd, constantLiteral) {
	return insertAfterLastOccurrence(getSourceNodes(source), constantLiteral, fileToAdd, 0);
}
exports.addFeatureRouteForLayout = addFeatureRouteForLayout;
/**
 ...
 */
function addRedirectRouteDeclarationToModule(source, fileToAdd, routesName, routeLiteral) {
	...
	let routesArr = findNodes(routesVar, ts.SyntaxKind.ArrayLiteralExpression, 1)[0];
	if (routesName == 'children') {
		...
		routesArr = findNodes(firstChild, ts.SyntaxKind.ArrayLiteralExpression, 1)[0];
	}
...
exports.addRedirectRouteDeclarationToModule = addRedirectRouteDeclarationToModule;
/**
 * Adds a new route declaration to a router module (i.e. has a RouterModule declaration)
 */
function addRouteDeclarationToModule(source, fileToAdd, routesName, routeLiteral) {
	let routesVar = source.statements.filter(ts.isVariableStatement).find((v) => {
		return v.declarationList.declarations[0].name.getText() === routesName;
	});
	if (!routesVar) {
 ...
 */
function getStoreModuleDeclaration(source) {
	const result = getDecoratorMetadata(source, 'NgModule', '@angular/core');
	const node = result[0];
	const matchingProperties = getMetadataField(node, 'imports');
	if (!matchingProperties) {
	...
	return arrLiteral.elements
		.filter((el) => el.kind === ts.SyntaxKind.CallExpression)
		.find((el) => el.getText().startsWith('StoreModule'));
}
exports.getStoreModuleDeclaration = getStoreModuleDeclaration;
 ...
 */
function addStateDeclarationToModule(source, fileToAdd, reducerLiteral) {
	...
	if (ts.isObjectLiteralExpression(statesArg)) {
		...
		reducer = `,${routeText}`;
		return [new change_1.InsertChange(fileToAdd, insertPos, reducer)];
	} else {
		throw new Error('error object');
	}
}
exports.addStateDeclarationToModule = addStateDeclarationToModule;
