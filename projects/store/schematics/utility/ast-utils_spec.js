...
function applyChanges(path, content, changes) {
	...
	return (0, test_1.getFileContent)(tree, path);
}
describe('ast utils', () => {
	let modulePath;
	let moduleContent;
	...
	it('should add export to module', () => {
		...
		const changes = (0, ast_utils_1.addExportToModule)(source, modulePath, 'FooComponent', './foo.component');
	...
	it('should add declarations to module if not indented', () => {
		moduleContent = core_1.tags.stripIndents`${moduleContent}`;
		const source = getTsSource(modulePath, moduleContent);
		const changes = (0, ast_utils_1.addDeclarationToModule)(source, modulePath, 'FooComponent', './foo.component');
		const output = applyChanges(modulePath, moduleContent, changes);
		expect(output).toMatch(/import { FooComponent } from '.\/foo.component';/);
		expect(output).toMatch(/declarations: \[\nAppComponent,\nFooComponent\n\]/);
	});
	it('should add declarations to module when PropertyAssignment is StringLiteral', () => {
		moduleContent = core_1.tags.stripIndents`
		import { BrowserModule } from '@angular/platform-browser';
		import { NgModule } from '@angular/core';
		import { AppComponent } from './app.component';
	...
	it('should add metadata', () => {
		const source = getTsSource(modulePath, moduleContent);
		const changes = (0, ast_utils_1.addSymbolToNgModuleMetadata)(source, modulePath, 'imports', 'HelloWorld');
	...
	it('should add into providers metadata in new line ', () => {
		...
		`;
		const source = getTsSource(modulePath, moduleContent);
		const changes = (0, ast_utils_1.addProviderToModule)(source, modulePath, 'LogService', './log.service');
		const output = applyChanges(modulePath, moduleContent, changes);
	...
	describe('insertAfterLastOccurrence', () => {
		...
		it('should work for the default scenario', () => {
			...
			const arrayNode = (0, ast_utils_1.findNodes)(
				source.getChildren().shift(),
				ts.SyntaxKind.ArrayLiteralExpression
			);
			const elements = arrayNode.pop().elements;
			const change = (0, ast_utils_1.insertAfterLastOccurrence)(
				elements,
				`, 'bar'`,
				filePath,
				elements.pos,
	...
	describe('findNodes', () => {
		...
		describe('when `recursive` is not set', () => {
			...
			it('should return node excluding nested nodes', () => {
				const source = getTsSource(filePath, fileContent);
				const paNodes = (0, ast_utils_1.findNodes)(
					source,
